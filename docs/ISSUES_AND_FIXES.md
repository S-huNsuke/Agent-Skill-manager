...
拆分 `bindings.go` 需要处理：
1. 59 个公开方法的分类和移动
2. 多个辅助函数（`healthRank`, `readSkillDescription`, `extractSkillNamesFromRecords` 等）
3. 类型定义和常量
4. 复杂的依赖关系
...
**建议的拆分方案（供后续参考）：**
- `agents_bindings.go` - 代理和技能操作（约15个方法）
- `store_bindings.go` - 商店和同步（约9个方法）
- `projects_bindings.go` - 项目和技能组（约10个方法）
- `assistant_bindings.go` - AI 助手（约7个方法）
- `settings_bindings.go` - 设置和诊断（约8个方法）
- `bindings.go` - 保留核心方法（约10个方法）
- `bindings_helpers.go` - 辅助函数和类型定义

**涉及文件：**
- `internal/app/bindings.go` → 拆分为 7 个文件

**预计工作量：** 3-4 小时（包括测试验证）

**决策时间：** 2026-05-07 17:30
