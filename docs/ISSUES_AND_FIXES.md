...
**修复方案：**
拆分为多个文件：
- `agents_bindings.go` - 代理相关（约15个方法）
- `store_bindings.go` - 商店相关（约10个方法）
- `projects_bindings.go` - 项目相关（约12个方法）
- `assistant_bindings.go` - AI 助手相关（约7个方法）
- `settings_bindings.go` - 设置相关（约7个方法）
- `bindings.go` - 保留核心方法（约8个方法）

**决策：**
这是一个大型重构任务，需要仔细提取每个方法及其依赖。建议作为独立的重构任务，在 P1 其他问题修复完成后再处理。
...
- [x] P0 问题：4/4 完成 (100%) ✅
- [x] P1 问题：3/4 完成 (75%) - P1-7 待定（大型重构）
- [ ] P2 问题：0/3 完成
...
**完成时间：** 2026-05-07 17:18

**注：** P1-7（拆分 bindings.go）是大型重构任务，建议作为独立项目处理
